/******/ (() => { // webpackBootstrap
/*!**************************************************!*\
  !*** ./src/block_CC2-829_fade-slider/js/view.js ***!
  \**************************************************/
...
const changeActiveSlide = (wrap, newIndex) => {
  const slide = wrap.querySelector('.slider-wrap'),
    totalWrap = wrap.querySelector('.total-wrap');
  const hasStepper = slide.dataset.stepper === 'true',
    hasTotal = slide.dataset.total === 'true';
  ...
  if (newIndex === undefined) {
    ...
    if (index + 1 >= itemArr.length) index = 0;else index++;
  } else {
    index = newIndex;
  }
...
fadeSlider.forEach(slider => {
  ...
  const next = slider.querySelector('.next-slide'),
    prev = slider.querySelector('.prev-slide');
  if (next && prev) {
    ...
    prev.addEventListener('click', () => {
      ...
      if (index - 1 < 0) index = wrap.children.length - 1;else index--;
